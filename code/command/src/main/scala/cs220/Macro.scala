...
object Macro {
  ...
  // A command for turning the fan high
  ...
  // A command for turning the fan off
  ...
  // ADDITION: A command for multiple commands!
  class MacroCommand(cmds: List[Command]) extends Command {
    def execute {
  ...
  // ADDITION: macro commands!
  val stuffOn   = new MacroCommand(
    List(lightOn, doorOpen, fanHigh)
  )

  val stuffOff   = new MacroCommand(
    List(lightOff, doorClose, fanOff)
  )

  // Create the new remote
  val remote = new RemoteControl(4)
  remote.setCommand(0, lightOn, lightOff)
  remote.setCommand(1, doorOpen, doorClose)
  remote.setCommand(2, fanHigh, fanOff)
  // ADDITION: add it to remote
  remote.setCommand(3, stuffOn, stuffOff)
}
